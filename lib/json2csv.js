...
function createColumnContent(params, str) {
  params.data.forEach(function (dataElement) {
    ...
    if (dataElement && Object.getOwnPropertyNames(dataElement).length > 0) {
      ...
      params.fields.forEach(function (fieldElement) {
        ...
        if (val !== undefined) {
          ...
          if(Array.isArray(val)){
            var arrValues = [];
            val.forEach(function(valArrElem){
              //check if array elements are objects
              if(typeof valArrElem === 'object'){
                arrValues.push(JSON.stringify(valArrElem));
              }else{
                arrValues.push(valArrElem);
              }
            });
            stringifiedElement = JSON.stringify(arrValues.join(','));
          }else{
      ...
      if (str !== '') {
        str += (params.eol || eol) + line;
      } else {
        str = line + (params.eol || eol);
      }
    }
  });
